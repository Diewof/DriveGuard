...
class RoughRoadDetector extends BaseDetector {
  ...
  double calculateConfidence(List<SensorReading> eventReadings) {
    ...
    // Factor 1: Número de impactos (50%) - INCREMENTADO de 40%
    // Más picos = mayor confianza de camino irregular
    final peakScore = (_detectedPeaks.length / 10.0).clamp(0.0, 1.0);
    confidence += peakScore * 0.5;

    // Factor 2: Irregularidad (35%) - INCREMENTADO de 30%
    // Mayor variabilidad = camino más irregular
    ...
    confidence += irregularityScore * 0.35;

    // Factor 3: Bonus por correlación con giroscopio (15%) - REDUCIDO de 30%
    // OPCIONAL: Si hay oscilaciones en pitch, suma confianza extra
    // Pero NO es requisito obligatorio (puede haber baches sin pitch)
    ...
    confidence += gyroScore * 0.15;

    // ignore: avoid_print
    print('[$detectorName] 📊 Confianza calculada: $confidence '
        '(peaks=${peakScore * 0.5}, irregularity=${irregularityScore * 0.35}, gyro=${gyroScore * 0.15})');
